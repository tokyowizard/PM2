...
// Singleton pm2 instance
//
// TODO: autoconnect breaks tests, because they
//       connect to pm2 in a different way
var PM2                  = require('../')({noautoconnect: true});
...
CLI.pm2Init = function() {
  if (!fs.existsSync(cst.DEFAULT_FILE_PATH)) {
...
CLI.getVersion = function(cb) {
  PM2.version(function(err, version) {
...
CLI._operate = function(action_name, process_name, cb) {
  function logger(command, args) {
    var m = command.match(/^(reload|restart|start|stop|delete)ProcessId$/);
    if (m) {
      printOut(cst.PREFIX_MSG + m[1] + ' process with id=%d', args.id != null ? args.id : args);
    }
  }
  ...
  if (process_name === 'all') {
    ...
    var spec = { name: process_name };
  } else {
    var spec = { id: process_name };
  }

  PM2.addListener('command', logger);
  PM2[action_name](spec, function(err, list) {
    PM2.removeListener('command', logger);
    if (!err && list.length == 0) {
      err = cst.PREFIX_MSG_ERR + 'No processes '
            + (Object.keys(spec).length ? 'found with ' : 'are running ')
            + Object.keys(spec).map(function(k) {
                return k + '=' + spec[k]
              }).join(' && ')
    }
    if (err) {
    ...
    // welcome to the localization hell
    // "delete" -> "delet" + "ed", "stop" -> "stopp" + "ed"
    var _action = action_name.replace(/e$/, '').replace(/p$/, 'pp') + 'ed'
    var _es = list.length > 1 ? 'es' : '';
    printOut(cst.PREFIX_MSG + 'Successfully %s %s process%s', _action, list.length, _es);
...
CLI._restart = function(process_name, cb) {
  CLI._operate('restart', process_name, cb);
};
...
CLI._delete = function(process_name, cb) {
  CLI._operate('delete', process_name, cb);
};
...
CLI._stop = function(process_name, cb) {
  CLI._operate('stop', process_name, cb);
};
