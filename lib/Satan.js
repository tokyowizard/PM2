  ...
  Satan.pingDaemon(function(ab) {
    ...
    if (ab == false) {
      ...
      return Satan.launchDaemon(function(err, child) {
        if (err) {
          console.log(cst.PREFIX_MSG + ' Satan.launchDaemon errored.');
          console.error(err);
          return cb ? cb(err) : process.exit(cst.ERROR_EXIT);
        }
        console.log(cst.PREFIX_MSG + 'Success');
        // Launch RPC
        console.log(cst.PREFIX_MSG + ' we will return Satan.launchRPC');
        return Satan.launchRPC(cb);
      });
    }
    // Else just start the PM2 client side (RPC)
    console.log(cst.PREFIX_MSG + ' Starting the PM2 client side');
    return Satan.launchRPC(cb);
  });
};
...
Satan.remoteWrapper = function() {
  ...
  function sendReady() {
    ...
    console.log('I am checking to see if the rpv socket is ready...');
    if (rpc_socket_ready == true && pub_socket_ready == true) {
      console.log('I am check typeof process.send before sending...');
      if (typeof(process.send) === 'function') {
        console.log('I am gonna send stuff now...');
        process.send({
          online      : true,
          success     : true,
          pid         : process.pid,
  ...
  God.bus.onAny(function interactionBroadcast(data_v) {
    ...
    pub.emit(this.event, Utility.serialize(data_v));
    data_v = null;
  });
};
...
Satan.launchDaemon = function launchDaemon(cb) {
  ...
  console.log(cst.PREFIX_MSG + ' Launching daemon here with Satan.launchDaemon');
  ...
  console.log(cst.PREFIX_MSG + ' Sending stderr stdout if travis');
  
  if (process.env.TRAVIS) {
    // Redirect PM2 internal err and out to STDERR STDOUT when running with Travis
    console.log(cst.PREFIX_MSG + ' OH DAMN, WE ARE TRAVIS');
    out = 1;
    err = 2;
  }
  else {
    console.log(cst.PREFIX_MSG + ' WE ARE NOT TRAVIS');
    out = fs.openSync(cst.PM2_LOG_FILE_PATH, 'a'),
    err = fs.openSync(cst.PM2_LOG_FILE_PATH, 'a');
  }

  console.log(cst.PREFIX_MSG + ' Adding node argus depending on PM2_NODE_OPTIONS env. Should be nothing.');
  ...
  console.log(cst.PREFIX_MSG + ' Setting var child');
  ...
  console.log(cst.PREFIX_MSG + ' making a function onError(e) here');
  ...
  console.log(cst.PREFIX_MSG + ' going to child.once(error, onError) here');
  
  child.once('error', onError);

  console.log(cst.PREFIX_MSG + ' going to child.unref() here');
  
  child.unref();

  console.log(cst.PREFIX_MSG + ' going to child.once(message, fuction (msg)) here');
  
  child.once('message', function(msg) {
    debug('PM2 daemon launched', msg);
    
    console.log(cst.PREFIX_MSG + ' going to child.removeListener(error, onError); here');
    child.removeListener('error', onError);
    
    console.log(cst.PREFIX_MSG + ' going to child.disconnect(); here');
    child.disconnect();

    console.log(cst.PREFIX_MSG + ' going to InteractorDaemonizer.launchAndInteract({}, fuction(err, data){} here');
